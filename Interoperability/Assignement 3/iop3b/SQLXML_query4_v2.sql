...
SELECT xmlroot(
       ...
       xmlelement(name robot_info, xmlattributes(r.robot_id as robot_id), 
	   		xmlforest(r.rname, r.rtype, r.rfunction)),
	   xmlelement(name delivery_info,
	   xmlelement(name delivery, xmlattributes(d.delivery_id as delivery_id), 
	   		xmlforest(d.ddate, d.vendor, d.damount)),
	   xmlelement(name stock_replenishment, 
	   xmlelement(name stock_info, xmlattributes(s.stock_id as stock_id), 
	   		xmlforest(s.ingredient_id, s.scurrent, s.snew, s.smaxcapa))))))
