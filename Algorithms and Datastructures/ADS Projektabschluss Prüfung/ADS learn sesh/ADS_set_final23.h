...
public:
  ...
  const_iterator y() const {
    if(!current_size)
      return end();
    return const_iterator(table[this->locate_first()].head, table, locate_first(), row_count, true, current_size, locate_first());
  }
...
public:
  ...
  explicit Iterator(Node* e = nullptr, Hashtable* table = nullptr, size_type index = 0, size_type row_count = N, bool special_mode = false, size_type number_elements = 0, size_type first_index = 0) : e{e}, table{table}, index{index}, row_count{row_count}, special_mode{special_mode}, number_elements{number_elements}, first_index{first_index} {
    if(special_mode == true){
      for(size_type i=0; i<number_elements-1; i++){
        getNext();
      }
    }
  }
  ...
  Iterator &operator++(){
    if(special_mode == false){
      ...
      //letting the iterator running backwards
      while(counter < number_elements){
        e = table[first_index].head;
        index = first_index;
        for(size_type i=0;i < number_elements-counter; i++){
          getNext();
        }
        counter++;
        return *this;
      }
      if(counter == number_elements){
        e = table[first_index].head;
        index = first_index;
        counter++;
        return *this;
