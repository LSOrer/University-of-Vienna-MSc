...
public:
  ...
  explicit Iterator(Node* ne = nullptr, Hashtable* table = nullptr, size_type index = 0, size_type row_count = N, bool special_mode = false, size_type number_elements = 0) {
    this->e = ne;
    this->table = table;
    this->index = index;
  ...
  Iterator &operator++(){
    if(special_mode == false){
      ...
      counter++;
      if(counter < (number_elements-(number_elements/4))){
        return getNext();
      } else {
        e = nullptr;
        return *this;
