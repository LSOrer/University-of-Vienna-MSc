...
public:
  ...
  const_iterator y(size_t m) const {
    if(!current_size)
      return end();
    return const_iterator(table[this->locate_first()].head, table, locate_first(), row_count, true, current_size, locate_first(), m);
  }
