...
public:
  ...
  const_iterator begin() const {
    ...
    if(!current_size)
      return end();
    return const_iterator(table[this->locate_first()].head, table, locate_first(), row_count, true, current_size);
  }
...
public:
  ...
  explicit Iterator(Node* e = nullptr, Hashtable* table = nullptr, size_type index = 0, size_type row_count = N, bool special_mode = false, size_type number_elements = 0) : e{e}, table{table}, index{index}, row_count{row_count}, special_mode{special_mode}, number_elements{number_elements} {}
  ...
  Iterator &operator++(){
    if(special_mode == false){
      ...
      if(number_elements % 2 == 1){
        if(counter == 0){
          counter++;
          for(size_type i=0; i < number_elements/2-1;i++){
            getNext();
          }
        return getNext();
        }
  ...
  Iterator &getLastOdd(){
    for(size_type i=0; i < (number_elements/2);i++){
          getNext();
        }
        return *this;
      }
