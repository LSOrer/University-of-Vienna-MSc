...
public:
  ...
  const_iterator y() const {
    if(!current_size)
      return end();
    return const_iterator(table[this->locate_first()].head, table, locate_first(), row_count, true, locate_first(), current_size);
  }
...
private:
  ...
  size_type output_number = 0;
  size_type index_value = 0;
...
public:
  ...
  explicit Iterator(Node* e = nullptr, Hashtable* table = nullptr, size_type index = 0, size_type row_count = N, bool special_mode = false, size_type first_index = 0, size_type number_elements = 0) : e{e}, table{table}, index{index}, row_count{row_count}, special_mode{special_mode}, first_index{first_index}, number_elements{number_elements} {}
